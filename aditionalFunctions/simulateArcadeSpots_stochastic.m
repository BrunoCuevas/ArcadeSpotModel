function report = simulateArcadeSpots (parameters, verbose)
	...
	if isstruct(parameters)
		...
		if any(parameters_control == 0)
			...
			interaction_grill = single(0.27*parameters.B)	;
			bustrofedon_grill = single(0.27*parameters.BF)	;
			...
			for cycle = 1 : 1 : cycles_limit
				...
				for day = 1 : 1 : harvest
					...
					increase_infection_vector(:,1) = (interaction_grill * infectivity_matrix(:,1)) + (bustrofedon_grill*infectivity_matrix(:,1)*agricultor_mediated_spread) + [state(1).P(:)];
					increase_infection_vector(:,2) = (interaction_grill * infectivity_matrix(:,2)) + (bustrofedon_grill*infectivity_matrix(:,2)*agricultor_mediated_spread) + [state(2).P(:)];
					increase_infection_vector(:,3) = (interaction_grill * infectivity_matrix(:,3)) + (bustrofedon_grill*infectivity_matrix(:,3)*agricultor_mediated_spread) + [state(3).P(:)];
					...
					for individual = 1 : 1 : nsize  ^ 2
						...
						if all([state(1).A(individual), state(2).A(individual), state(3).A(individual)] == 1) == 1
							...
							state(1).x(individual) = triple_infection_control * (rand(1) < increase_infection_vector(individual, 1)) + state(1).x(individual);
							state(2).x(individual) = triple_infection_control * (rand(1) < increase_infection_vector(individual, 2)) + state(2).x(individual);
							state(3).x(individual) = triple_infection_control * (rand(1) < increase_infection_vector(individual, 3)) + state(3).x(individual);
